...
function getSavedSearch(request)
{
    ...
    var lowerBound = request.lower_bound;
    var upperBound = request.upper_bound;
    var lastId = lowerBound

    do{
        var tempItems = nlapiSearchRecord(recordType, searchId,
        [
            new nlobjSearchFilter('internalidnumber', null, 'greaterthan', lastId),
            new nlobjSearchFilter('internalidnumber', null, 'lessthan', lastId + upperBound + 1)
        ], null);
        if(tempItems)
        {
    ...
    }while(tempItems && accumulatedResults.length < (upperBound - lowerBound));
