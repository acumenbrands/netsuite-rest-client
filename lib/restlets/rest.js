    ...
    for(var item in recordData) {
    	try {
    		...
    		for(var field in item) {
    			record.setFieldValue(field, item[field]);
    		}
    		return(record);
...
function deleteRecords(request) {
	...
	*/
	var recordType  = request['record_type'];
	var internalIds = request['internal_id'];
	var results     = [];

	for(var id in internalIds) {
		try {
			nlapiDeleteRecord(recordType, internalId);
			results = results.concat([id, true]);
		}
		catch(exception) {
			results = results.concat([id, formatException(exception)]);
		}
