...
function upsertRecords(request) {
	...
	*/
    ...
    return writeResults.concat([nlapiSubmitRecord(recordData[0], doSourcing, ignoreMandatory)]);

    for(var record in recordData) {
    	try {
    		record = nlapiCreateRecord(recordType);
    		for(var field, value in record) {
    			record.
    		}
    		writeResults.concat([nlapiSubmitRecord(record, doSourcing, ignoreMandatory)]);
    	}
		catch(write_exception) {
			writeResults.concat([formatException(write_exception)]);
		}
    }
