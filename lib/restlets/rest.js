    ...
    for(var index = 0; index < recordData.length; index++) {
    	...
    	try {
    		...
    		for(var field in attributes) {
    			...
    			if(field=='line_items') {
    				for(var lineItemFieldName in attributes[field]) {
    					for(var index = 0; index < attributes[field][lineItemFieldName].length; index++) {
    						var lineItemIndex = record.getLineItemCount(lineItemFieldName) + 1;
		    				record.insertLineItem(lineItemFieldName, lineItemIndex);
    						for(lineItemField in attributes[field][lineItemFieldName][index]) {
    							record.setLineItemValue(lineItemFieldName,
    													lineItemField,
    													index+1,
    													attributes[field][lineItemFieldName][index][lineItemField]);
		    				}
    					}
    				}
    			}
