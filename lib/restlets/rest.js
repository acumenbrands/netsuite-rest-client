...
function populateLineItems(record, sublistChanges) {
    for(var sublistName in sublistChanges) {
        ...
        deletionValues  = sublistChanges[sublistName]['deletion_values'];
        indexesToDelete = [];

        deletionCount = 0;
        for(var deletionIndex in deletionValues) {
            deletionValue = deletionValues[deletionIndex];
            
            for(var lineItemIndex = 1; lineItemIndex < (record.getLineItemCount(sublistName) + 1); lineItemIndex++) {
                if(record.getLineItemValue(sublistName, matchField, (lineItemIndex - deletionCount)) == deletionValue) {
                    record.removeLineItem(sublistName, (lineItemIndex - deletionCount));
                    deletionCount++;
                }
            }
        }
        ...
        lineItemCount = record.getLineItemCount(sublistName) + 1;
        for(var lineItemIndex = 1; lineItemIndex < lineItemCount; lineItemIndex++) {
            ...
            for(var sublistItemDataIndex = 0; sublistItemDataIndex < sublistItems.length; sublistItemDataIndex++) {
                ...
                if(record.getLineItemValue(sublistName, matchField, lineItemIndex) == sublistItemData[matchField]) {
                    found = true;

                    for(var sublistItemField in sublistItemData) {
                        if(sublistItemField == matchField) { continue; }
                        record.setLineItemValue(sublistName, sublistItemField, lineItemIndex,
            ...
            if(!found) {
                newLineItemIndex = record.getLineItemCount(lineItemFieldName) + 1;
                record.insertLineItem(sublistName, newLineItemIndex);

                for(var sublistItemField in sublistItemData) {
                    if(sublistItemField == matchField) { continue; }
                    record.setLineItemValue(sublistName, sublistItemField, newLineItemIndex,
                                            sublistItemData[sublistItemField]);
                }
            }
        }
