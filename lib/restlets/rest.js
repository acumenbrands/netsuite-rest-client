...
function populateRecord(recordType, recordData) {
...
function upsertRecords(request) {
	...
	*/
	...
	var lineItemData    = request['line_items'];
    ...
    for(var index = 0; index < recordData.length; index++) {
    	...
    	try {
    		...
    		for(var field in lineItemData) {
    			var lineItemIndex = record.getLineItemCount(field) + 1;
    			record.insertLineItem(field, lineItemIndex);
    			for(var lineItemField in lineItemData[field]) {
    				record.setCurrentLineItemValue(field,
    											   lineItemField,
    											   lineItemIndex,
    											   lineItemData[lineItemField]);
    			}
    		}
...
function getHandler(request) {
    ...
    */
    var po = nlapiLoadRecord('PurchaseOrder', '3576232');
    return();
    try {
