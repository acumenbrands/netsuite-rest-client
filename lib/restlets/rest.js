...
function transformRecords(request) {
    ...
    */
    ...
    for(var sublistName in sublistChanges) {
        ...
        matchField = sublist['match_field'];
        lineItems  = sublist['line_items']
        ...
        for(var index = 1; index < (newRecord.getLineItemCount(sublistName) + 1); index++) {
            ...
            for(sublistItemDataIndex in lineItems) {
                sublistData = lineItems[sublistItemDataIndex];

                if(newRecord.getLineItemValue(sublistName, matchField, (index - deletionCount)) == sublistData[matchField]) {
                    found = true;
