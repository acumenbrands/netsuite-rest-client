 ...
 * Constants
*/
var METHOD_ATTRIBUTES = { 'initializeRecord': { 'access': 'GET',  'baseGovernance': 10 },
                          'loadRecord':       { 'access': 'GET',  'baseGovernance': 10 },
                          'getSavedSearch':   { 'access': 'GET',  'baseGovernance': 10 },
                          'searchRecords':    { 'access': 'POST', 'baseGovernance': 10 },
                          'upsertRecords':    { 'access': 'POST', 'baseGovernance': 20 },
                          'deleteRecords':    { 'access': 'POST', 'baseGovernance': 20 } }
...
function evalOperation(method, operation, request) {
    ...
    */
    if(method == METHOD_ATTRIBUTES[operation]['access']) {
...
function governanceCheck(operation, iterations) {
	 ...
	 * Params:
	 *              operation:  String name of the function name to be executed
	...
	*/
	...
	if(METHOD_ATTRIBUTES['operation']['baseGovernance']*iterations > governanceLimit) {
		return(false);
	}
	return(true);
}
...
function handler(request) {
	/*
	 * Description: Method to handle requests from a REST client
	 * Params:
	 *              request: Request object from the REST client
	 *
