	...
	*/
    ...
    for(var index = 0; index < recordData.length; index++) {
    	try {
    		...
    		for(var field in recordData[index]) {
    			record.setFieldValue(field, recordData[index][field]);
    		}
...
function deleteRecords(request) {
	 ...
	 *
	 * Return:      An array of ids paires with false, if no exception, and a formatted exception
	...
	*/
	...
	for(var index = 0; index < internalIds.length; index++) {
		try {
			nlapiDeleteRecord(recordType, internalIds[index]);
			results = results.concat([internalIds[index], false]);
		}
		catch(exception) {
			results = results.concat([internalIds[index], formatException(exception)]);
		}
	}
