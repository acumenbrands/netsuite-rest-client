...
function transformRecords(request) {
    ...
    */
    ...
    for(var sublistName in sublistChanges) {
        ...
        for(var index = 0; index < (newRecord.getLineItemCount(sublistName)+1); index++) {
            found             = false;
            deletionCount     = 0;
            lineItemsToRemove = [];
            ...
            for(sublistItemDataIndex in sublist) {
                ...
                if(newRecord.getLineItemValue(sublistName, 'id', index) == sublistData['id']) {
                    found = true;
                    for(var sublistItemField in sublistData) {
                        if(sublistItemField == 'id') { continue; }
                        newRecord.setLineItemValue(sublistName, sublistItemField, (index - deletionCount),
                                                   sublistData[sublistItemField]);
                    }
                }

                if(!found) { newRecord.removeLineItem(sublistName, (index - deletionCount)); }
            }
        }
    }
